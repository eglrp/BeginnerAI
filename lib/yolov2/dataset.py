...
class listDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.train:
            ...
            img, label = yolo_utils.ImageUtils.load_data_detection(imgpath, self.shape, jitter, hue, saturation, exposure)
            ...
            try:
                tmp = torch.from_numpy(yolo_utils.ImageUtils.read_truths_args(labpath, 8.0/img.width).astype('float32'))
            except Exception:
                tmp = torch.zeros(1,5)
