...
class ListDataset(torch.utils.data.Dataset):
    def __init__(self, root, list_file, train, transform, input_size):
        ...
        self.encoder = rn_utils.DataEncoder()
    ...
    def __getitem__(self, idx):
        ...
        # Data augmentation.
        if self.train:
            img, boxes = rn_utils.ImageUtils.random_flip(img, boxes)
            img, boxes = rn_utils.ImageUtils.random_crop(img, boxes)
            img, boxes = rn_utils.ImageUtils.resize(img, boxes, (size,size))
        else:
            img, boxes = rn_utils.ImageUtils.resize(img, boxes, size)
            img, boxes = rn_utils.ImageUtils.center_crop(img, boxes, (size,size))
