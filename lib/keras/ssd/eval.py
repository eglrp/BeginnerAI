...
class Evaluator:
    ...
    def __init__(self,
        ...
        if not isinstance(data_generator, k_data.DataGenerator):
    ...
    def predict_on_dataset(self,
        ...
        resize = lui.Resize(height=img_height,width=img_width, labels_format=self.gt_format)
        if data_generator_mode == 'resize':
            transformations = [convert_to_3_channels,
                               resize]
        elif data_generator_mode == 'pad':
            random_pad = k_utils.RandomPadFixedAR(patch_aspect_ratio=img_width/img_height, labels_format=self.gt_format)
            transformations = [convert_to_3_channels,
                               random_pad,
                               resize]
        else:
        ...
        for j in tr:
            ...
            if self.model_mode == 'training':
                # Decode.
                y_pred = k_utils.decode_detections(y_pred,
            ...
            y_pred = k_utils.apply_inverse_transforms(y_pred, batch_inverse_transforms)
    ...
    def match_predictions(self,
        ...
        for class_id in range(1, self.n_classes + 1):
            ...
            for i in tr:
                ...
                overlaps = k_utils.iou(boxes1=gt[:,[xmin_gt, ymin_gt, xmax_gt, ymax_gt]],
                               boxes2=pred_box,
                               coords='corners',
                               mode='element-wise',
                               border_pixels=border_pixels)
