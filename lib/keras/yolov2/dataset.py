...
def data_generator(imageFile, cfg):
    """
    ...
    target_image_size = (cfg["IMAGE_SIZE"], cfg["IMAGE_SIZE"])
    f = open(imageFile)
    count = 0
    while True:
        line = f.readline()
        ...
        while line:
            ...
            image = PIL.Image.open(data[0])
            image_size = [image.width, image.height]
            array = []
            for num in data[1:]:
                array.append(float(num))
            array = np.asarray(array)
            object_mask, object_value = convert_ground_truth(array,cfg, image_size=image_size)
            y1.append(array)
...
def read_image(path, image_size):
    ...
    if image_size is not None:
        resized_image = image.resize(
            tuple(reversed(image_size)), PIL.Image.BICUBIC)
        ...
        resized_image = image.resize(new_image_size, PIL.Image.BICUBIC)
        image_data = np.array(resized_image, dtype='float32')
...
def convert_ground_truth(box,cfg, image_size):
    """
