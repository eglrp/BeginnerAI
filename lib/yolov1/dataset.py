...
class yoloDataset(t.utils.data.Dataset):
    image_size = 448
    def __init__(self,CFG,train,transform):
        self.CFG = CFG
        self.root=self.CFG["DATA_PATH"]
        self.train = train
        ...
        with open(self.CFG["IMAGE_LIST_FILE"]) as f:
            lines  = f.readlines()
        ...
        for line in lines:
            ...
            # 赋值一张图的物体总数
            # 2007_000032.jpg 4 104 78 375 183 0 133 88 197 123 0 195 180 213 229 14 26 189 44 238 14
            '''
                splited[0] : 图片名称
                splited[1] ：包含物体的个数
                剩下的就是5个5个一组一共有splited[1]这么多组，这5个的意义就是x1,y1,x2,y2,cls_id
            '''
            ...
            # bbox  写入所有物体的坐标值
            self.boxes.append(t.Tensor(box)) # 每一行是一张图片内所有的box的坐标
            # label 写入标签
            self.labels.append(t.LongTensor(label)) # 每一行是一张图片内所有box的label
        # 数据集中图像总数
    ...
    def __getitem__(self,idx):
        ...
        h,w,_ = img.shape
        boxes /= t.Tensor([w,h,w,h]).expand_as(boxes) # 将box归一化，也就是将整个图片看做1*1的大小。
    ...
    def encoder(self,boxes,labels):
        ...
        return 7x7x30
        '''
        target = t.zeros((self.CFG["CELL_NUMS"],self.CFG["CELL_NUMS"],self.CFG["BOXES_EACH_CELL"] * 5 + len(self.CFG["CLASSES"])))
        cell_size = 1./self.CFG["CELL_NUMS"]
    ...
    def randomShift(self,bgr,boxes,labels):
        ...
        if random.random() <0.5:
            ...
            shift_xy = t.FloatTensor([[int(shift_x),int(shift_y)]]).expand_as(center)
            ...
            box_shift = t.FloatTensor([[int(shift_x),int(shift_y),int(shift_x),int(shift_y)]]).expand_as(boxes_in)
    ...
    def randomScale(self,bgr,boxes):
        ...
        if random.random() < 0.5:
            ...
            scale_tensor = t.FloatTensor([[scale,1,scale,1]]).expand_as(boxes)
    ...
    def randomCrop(self,bgr,boxes,labels):
        if random.random() < 0.5:
            ...
            center = center - t.FloatTensor([[x,y]]).expand_as(center)
            ...
            if(len(boxes_in)==0):
                return bgr,boxes,labels
            box_shift = t.FloatTensor([[x,y,x,y]]).expand_as(boxes_in)
