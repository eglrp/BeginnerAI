...
model = fcn_net.FCN16s(FCNConfig)
if FROM_TRAIN_ITER == 1:
    ...
    model.load_state_dict(torch.load("outputs/FCN16s_%03d.pth" % (FROM_TRAIN_ITER - 1)))
...
for epoch in (FROM_TRAIN_ITER, FCNConfig["EPOCHS"] + 1):
    ...
    torch.save(model.state_dict(), "outputs/FCN16s_%03d.pth" % epoch)
    ...
    misc.imsave("outputs/FCN16s_%03d.png" % epoch, decoded_crf)
