...
model = fcn_net.FCN32s(FCNConfig)
if FROM_TRAIN_ITER == 1:
    ...
    model.load_state_dict(torch.load("outputs/FCN32s_%03d.pth" % (FROM_TRAIN_ITER - 1)))
...
for epoch in (FROM_TRAIN_ITER, FCNConfig["EPOCHS"] + 1):
    ...
    misc.imsave("outputs/FCN32s_%03d.png" % epoch, decoded_crf)
