...
FROM_TRAIN_ITER = 1
data_set = frrn_data.VOCSegDataSet(CONFIG["DATA_PATH"], is_transform=True, augmentations=None,
                                   img_size=(CONFIG["IMAGE_SIZE"],CONFIG["IMAGE_SIZE"]))
train_loader = torch.utils.data.DataLoader(data_set,batch_size=1, shuffle=True)

model = frrn_net.FRRN(CONFIG)
if FROM_TRAIN_ITER > 1:
    model.load_state_dict(torch.load("outputs/FRRN_%03d.pth" % (FROM_TRAIN_ITER - 1)))
...
for epoch in (FROM_TRAIN_ITER, CONFIG["EPOCHS"] + 1):
    ...
    for batch_idx, (images, targets) in enumerate(train_loader):
        ...
        optim.zero_grad()
        score = model(images)
        loss = frrn_loss.cross_entropy2d(score, targets, size_average=False)
        total_loss += loss.item()

        bar.show(epoch, loss.item(), total_loss / (batch_idx + 1))

    torch.save(model.state_dict(), "outputs/FRRN_%03d.pth" % epoch)
    ...
    misc.imsave("outputs/FRRN_%03d.png" % epoch, decoded_crf)
